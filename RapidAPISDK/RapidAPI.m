...
{
    return [NSString stringWithFormat:@"https://webhooks.rapidapi.io/api/get_token?user_id=%@", [self buildUserID:package event:event]];
}

- (NSString*)buildSocketURL:(NSString*)token
{
    return [NSString stringWithFormat:@"wss://webhooks.rapidapi.io/socket/websocket?token=%@", token];
}
